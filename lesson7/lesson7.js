...
var food_timer;
var problem_timer; // Таймер для еды
...
function startGame() {
    ...
    //  setTimeout(createProblem,7000);
    setInterval(createProblem,7000);
}
...
function isBomb(unit) {
    ...
    // Если еда
    if (unit_classes.includes('food-problem')) {
        check = true;
...
function createProblem() {
    ...
    while (!foodCreated) { //пока еду не создали
        ...
        // проверка на змейку
        if (!food_cell_classes.includes('snake-unit')&&('food-unit')) {
            var classes = '';
            for (var i = 0; i < food_cell_classes.length; i++) {
                classes += food_cell_classes[i] + ' ';
            }

            food_cell.setAttribute('class', classes + 'food-problem');
            foodCreated = true;
        }
    }
}
